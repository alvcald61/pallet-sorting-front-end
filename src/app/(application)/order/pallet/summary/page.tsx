...
export default function PalletSummaryPage() {
  const router = useRouter();
  const { clearDraft } = useOrderDraft("pallet");
  const { palletOrder, address, userId } = useOrderStore();
    ...
    >
      <Stack gap="lg">
        <Paper shadow="sm" p="lg" radius="md" withBorder>
          ...
          {/* Items Summary */}
          <div>
            <Title order={5} mb="sm">Pallets ({palletOrder.length})</Title>
            <Stack gap="xs">
              {palletOrder.map((item, index) => (
                <Group key={item.tempId} justify="space-between" p="sm" style={{ backgroundColor: "#f8f9fa", borderRadius: 8 }}>
                  <div>
                    <Text fw={600}>Pallet #{index + 1}</Text>
                    <Text size="sm" c="dimmed">
                      {item.length}m x {item.width}m x {item.height}m · {item.weight} kg · Cantidad: {item.quantity}
                    </Text>
                  </div>
                  <Badge>{(item.length * item.width * item.height * item.quantity).toFixed(2)} m³ total</Badge>
                </Group>
              ))}
            </Stack>
